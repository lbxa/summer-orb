...
const BASE_BLUE = new Color('#0f4c81');
const HIGHLIGHT_BLUE = new Color('#4fc3f7');
const CORE_GLOW_BLUE = new Color('#1e88e5');
const PARTICLE_BLUE = new Color('#80d8ff');
const BACKGROUND_COLOR = new Color('#020814');
...
const vertexShader = /* glsl */ `
  ...
  void main() {
    ...
    float displacement = snoise(normal * noiseScale + time) * 0.15;
    float bassPush = uBassAmplitude * 0.35;
    float spectrumLean = uSpectrumTilt * 0.1;
    vec3 warpedPosition = position + normal * (displacement + bassPush + spectrumLean);
...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    float intensity = pow(1.0 - clamp(length(vPosition.xy) / 2.5, 0.0, 1.0), 1.2);
    float fresnel = pow(1.0 - dot(vNormal, normalize(-vPosition)), 2.2);
    float glow = mix(0.4, 1.0, uAmplitude);
    vec3 color = mix(uBaseColor, uHighlightColor, fresnel + uAmplitude * 0.6);
    color *= (intensity + fresnel * 1.2) * glow;
...
const shellFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    float opacity = mix(0.15, 0.35, uAmplitude + fresnel * 0.5);
    vec3 color = mix(uBaseColor, uHighlightColor, rim);
...
class VoiceOrbController implements VoiceOrbSceneHandle {
  ...
  constructor(options: VoiceOrbSceneOptions) {
    ...
    this.bloomPass = new UnrealBloomPass(new Vector2(width, height), 0.6, 0.4, 0.75);
  ...
  private setupSceneObjects(): void {
    const coreGeometry = new SphereGeometry(1, 128, 128);
    const core = new Mesh(coreGeometry, this.coreMaterial);
    this.scene.add(core);

    const shellGeometry = new SphereGeometry(1.08, 128, 128);
  ...
  private setupLights(): void {
    const ambient = new AmbientLight('#0a1a2f', 0.35);
    const backLight = new PointLight('#1b2a4a', 1.0, 10);
  ...
  private updateUniforms(time: number, amplitude: number, bass: number, tilt: number): void {
    ...
    this.bloomPass.strength = 0.6 + amplitude * 0.8 * intensity;
  }

  private updateCamera(delta: number, tilt: number): void {
    const targetX = tilt * 0.25;
    const targetY = Math.sin(this.clock.elapsedTime * 0.2) * 0.08;
